...
while True:
    ...
    objects_in_hands, key_hand, distance_threshold, left_kp, right_kp = detect_objects_in_hands(frame,
                                                                                                pose_results)
    # Сначала отрисовываем все объекты, которые не находятся в руках
    for box, conf, cls in zip(results.boxes.xyxy, results.boxes.conf, results.boxes.cls):
        class_name = object_model.names[int(cls)]
        if class_name not in ignored_classes:
            x1, y1, x2, y2 = map(int, box.tolist())
            ...
            if objects_in_hands:
                ...
                if not is_in_hands:
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 247, 239), 2)  # Зеленый прямоугольник
                    label = f"{class_name} ({conf:.2f})"
                    cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 247, 239), 2)
            else:
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 247, 239), 2)  # Зеленый прямоугольник
                label = f"{class_name} ({conf:.2f})"
                cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 247, 239), 2)
