...
status_icons = { True: get_status_icon(True), False: get_status_icon(False), }
...
def create_status_row(name):
    ...
    status_labels[ name ] = { "row": row, "icon": icon_label, "label": name_label }
...
while True:
    ...
    camera_wait = 0
    # if time_count > 0:
    #     print(f"[INFO] Время выполнения всего цикла: {time.time() - start_time:.6f} секунд, цикл: {time_count}")
    #     print("--------------------------------------------------------------")
    #     if (time.time() - start_time) > 0.3:  # Если цикл занял больше 0.3 секунд
    #         print(f"[WARNING] Долгий цикл: {time.time() - start_time:.6f} секунд")
    #         print("--------------------------------------------------------------")
        ...
        if assigned_body:
            ...
            if track_id in tracked_people:
                    ...
                    if now - tracked_people[ track_id ].get("last_check2", 0) >= 1.0:
                        ...
                        if min_dist < recognition_t:
                            ...
                            if new_name != current_name and new_name != "Unknown":
                                ...
                                still_present = any( p[ 'name' ] == current_name and bbox_intersects_polygon(p[ 'bbox' ], zone_bbox) for track_id2, p in tracked_people.items() if track_id2 != track_id )
                ...
                if track_id not in pending_faces:
                    pending_faces[ track_id ] = { 'encoding': encoding, 'first_seen': now, 'bbox': assigned_body[ 'bbox' ], 'face_bbox': (x, y, w, h) }
                else:
                    ...
                    if min_dist < recognition_t:
                        ...
                        tracked_people[ track_id ] = { 'name': matched_name, 'bbox': assigned_body[ 'bbox' ], 'face_bbox': (x, y, w, h), 'last_seen': now, 'first_seen': now, 'last_retry': now, 'last_check1': now, 'last_check2': now, 'IDs': 0, 'confidence_window': deque(maxlen=5) }
                        del pending_faces[ track_id ]
                    elif now - pending_faces[ track_id ][ 'first_seen' ] >= 3.0:
                        matched_name = "Unknown"
                        tracked_people[ track_id ] = { 'name': matched_name, 'bbox': assigned_body[ 'bbox' ], 'face_bbox': (x, y, w, h), 'last_seen': now, 'first_seen': now, 'last_retry': now, 'last_check1': now, 'last_check2': now, 'confidence_window': deque(maxlen=5) }
                        del pending_faces[ track_id ]
